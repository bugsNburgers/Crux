...
const buildKeyframes = (from: AnimationSnapshot, steps: AnimationSnapshot[]) => {
  ...
  keys.forEach(k => { keyframes[k as string] = [from[k], ...steps.map(s => s[k])] })
  return keyframes
}
  ...
  return (
    <p ref={ref} className={className} style={{ display: 'flex', flexWrap: 'wrap' }}>
      {elements.map((segment, index) => {
        ...
        const spanTransition = { duration: totalDuration, times, delay: (index * delay) / 1000, ease: easing as Easing }
        return (
          <motion.span
